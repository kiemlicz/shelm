...
class HelmRegistryLoginException(output: ProcessOutput, exitCode: Int)
  extends RuntimeException(s"Registry login failed with exit code: $exitCode, ensure login data exists or provide the correct credentials") {
  /**
